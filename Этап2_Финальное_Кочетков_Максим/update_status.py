...
Описание:
- Содержит функциональность для проверки и изменения статуса заметки.
- Позволяет пользователю выбирать новый статус из предложенного списка.
- Поддерживает обработку некорректного ввода.

Функциональность:
1. Показывает текущий статус заметки.
2. Предлагает изменить статус на один из предложенных.
3. Обрабатывает некорректный ввод и запрашивает повторный ввод.
"""

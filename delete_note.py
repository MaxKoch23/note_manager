...
Файл: delete_note.py
Описание:
- Содержит функциональность для удаления заметок из списка.
- Пользователь может удалить заметку по имени пользователя или заголовку.
- После удаления обновленный список заметок отображается.

Функциональность:
1. Удаление заметок по критериям:
   - Имя пользователя.
   - Заголовок заметки.
2. Запрос подтверждения перед удалением.
3. Отображение оставшихся заметок после удаления.
"""
...
def delete_notes(notes):
   ...
   # Отображение оставшихся заметок
    if notes:
